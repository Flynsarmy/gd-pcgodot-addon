...
func test_broadcast_stream_expands_to_data_size() -> void:
	...
	d.registerStream("density", PackedFloat32Array([0.25]), FlowDataScript.DataType.Float)
	var node := _run([d], s)
	assert_str(node.err).is_empty()
	var out := _output(node)
	assert_object(out).is_not_null()
	...
	assert_float(stream.container[0]).is_equal_approx(0.75, 0.05)
	assert_float(stream.container[1]).is_equal_approx(0.75, 0.05)
	assert_float(stream.container[2]).is_equal_approx(0.75, 0.05)
	node.free()
...
func test_single_element_data() -> void:
	...
	assert_float(stream.container[0]).is_equal_approx(0.7, 0.05)
	node.free()
